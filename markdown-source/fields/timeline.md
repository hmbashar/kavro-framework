...
**Field type:** `timeline`  
**Renderer class:** `Timeline`  
**Aliases using the same renderer:** None

## Purpose

Timeline entries field.
...
KAVRO::createSection( $prefix, array(
    ...
    'fields' => array(
        array(
            ...
            'type' => 'timeline',
            'title' => 'Timeline',
            'subtitle' => 'Timeline field example.',
            'desc' => 'This is a documented timeline field example.',
...
KAVRO::createSection( $nav_menu_prefix, array(
    'title'  => 'Timeline Menu Item Field',
    'fields' => array(
        array(
            'id'       => 'demo_timeline',
            'type'     => 'timeline',
...
```php
$value = kavro_get_option( $prefix, 'demo_timeline', null );
```

### From Metabox

```php
$value = kavro_get_post_meta( get_the_ID(), $metabox_prefix, 'demo_timeline', null );
```
...
```php
$value = get_theme_mod( 'demo_timeline', null );
```

### From Taxonomy Options

```php
$value = kavro_get_term_meta( $term_id, $taxonomy_prefix, 'demo_timeline', null );
```

### From Profile/User Options

```php
$value = kavro_get_user_meta( $user_id, $profile_prefix, 'demo_timeline', null );
```

### From Nav Menu Options

```php
$value = kavro_get_nav_menu_item_meta( $menu_item_id, $nav_menu_prefix, 'demo_timeline', null );
```

### From Widget Options

```php
$value = kavro_get_widget_option( $widget_id, $widget_prefix, 'demo_timeline', null );
```

### From Comment Options

```php
$value = kavro_get_comment_meta( $comment_id, $comment_prefix, 'demo_timeline', null );
```
...
```php
$value = isset( $atts['demo_timeline'] ) ? $atts['demo_timeline'] : null;
```
