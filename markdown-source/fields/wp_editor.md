...
**Field type:** `wp_editor`  
**Renderer class:** `Wysiwyg`  
**Aliases using the same renderer:** `wysiwyg`

## Purpose

Alias of WYSIWYG editor field.
...
| Attribute | Type | Description |
|---|---:|---|
| `media_buttons` | mixed | Field-specific setting used by the `wp_editor` renderer. |
...
KAVRO::createSection( $prefix, array(
    'title'  => 'Wp Editor Example',
    'fields' => array(
        array(
            'id' => 'demo_wp_editor',
            'type' => 'wp_editor',
            'title' => 'Wp Editor',
            'subtitle' => 'Wp Editor field example.',
            'desc' => 'This is a documented wp_editor field example.',
...
KAVRO::createSection( $nav_menu_prefix, array(
    'title'  => 'Wp Editor Menu Item Field',
    'fields' => array(
        array(
            'id'       => 'demo_wp_editor',
            'type'     => 'wp_editor',
...
```php
$value = kavro_get_option( $prefix, 'demo_wp_editor', null );
```

### From Metabox

```php
$value = kavro_get_post_meta( get_the_ID(), $metabox_prefix, 'demo_wp_editor', null );
```
...
```php
$value = get_theme_mod( 'demo_wp_editor', null );
```

### From Taxonomy Options

```php
$value = kavro_get_term_meta( $term_id, $taxonomy_prefix, 'demo_wp_editor', null );
```

### From Profile/User Options

```php
$value = kavro_get_user_meta( $user_id, $profile_prefix, 'demo_wp_editor', null );
```

### From Nav Menu Options

```php
$value = kavro_get_nav_menu_item_meta( $menu_item_id, $nav_menu_prefix, 'demo_wp_editor', null );
```

### From Widget Options

```php
$value = kavro_get_widget_option( $widget_id, $widget_prefix, 'demo_wp_editor', null );
```

### From Comment Options

```php
$value = kavro_get_comment_meta( $comment_id, $comment_prefix, 'demo_wp_editor', null );
```
...
```php
$value = isset( $atts['demo_wp_editor'] ) ? $atts['demo_wp_editor'] : null;
```
