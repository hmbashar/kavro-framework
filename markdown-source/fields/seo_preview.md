...
**Field type:** `seo_preview`  
**Renderer class:** `SeoPreview`  
**Aliases using the same renderer:** None

## Purpose

Search snippet preview settings.
...
KAVRO::createSection( $prefix, array(
    'title'  => 'Seo Preview Example',
    'fields' => array(
        array(
            'id' => 'demo_seo_preview',
            'type' => 'seo_preview',
            'title' => 'Seo Preview',
            'subtitle' => 'Seo Preview field example.',
            'desc' => 'This is a documented seo_preview field example.',
...
KAVRO::createSection( $nav_menu_prefix, array(
    'title'  => 'Seo Preview Menu Item Field',
    'fields' => array(
        array(
            'id'       => 'demo_seo_preview',
            'type'     => 'seo_preview',
...
KAVRO::createSection( $widget_prefix, array(
    'title'  => 'Seo Preview Widget Field',
    'fields' => array(
        array(
            'id'       => 'demo_seo_preview',
            'type'     => 'seo_preview',
...
KAVRO::createSection( $comment_prefix, array(
    'title'  => 'Seo Preview Comment Field',
    'fields' => array(
        array(
            'id'       => 'demo_seo_preview',
            'type'     => 'seo_preview',
...
```php
$value = kavro_get_option( $prefix, 'demo_seo_preview', null );
```

### From Metabox

```php
$value = kavro_get_post_meta( get_the_ID(), $metabox_prefix, 'demo_seo_preview', null );
```
...
```php
$value = get_theme_mod( 'demo_seo_preview', null );
```

### From Taxonomy Options

```php
$value = kavro_get_term_meta( $term_id, $taxonomy_prefix, 'demo_seo_preview', null );
```

### From Profile/User Options

```php
$value = kavro_get_user_meta( $user_id, $profile_prefix, 'demo_seo_preview', null );
```

### From Nav Menu Options

```php
$value = kavro_get_nav_menu_item_meta( $menu_item_id, $nav_menu_prefix, 'demo_seo_preview', null );
```

### From Widget Options

```php
$value = kavro_get_widget_option( $widget_id, $widget_prefix, 'demo_seo_preview', null );
```

### From Comment Options

```php
$value = kavro_get_comment_meta( $comment_id, $comment_prefix, 'demo_seo_preview', null );
```
...
```php
$value = isset( $atts['demo_seo_preview'] ) ? $atts['demo_seo_preview'] : null;
```
