...
**Field type:** `box_model`  
**Renderer class:** `BoxModel`  
...
KAVRO::createSection( $prefix, array(
    ...
    'fields' => array(
        array(
            ...
            'type' => 'box_model',
            'title' => 'Box Model',
            'subtitle' => 'Box Model field example.',
            'desc' => 'This is a documented box_model field example.',
...
KAVRO::createSection( $nav_menu_prefix, array(
    'title'  => 'Box Model Menu Item Field',
    'fields' => array(
        array(
            'id'       => 'demo_box_model',
            'type'     => 'box_model',
...
```php
$value = kavro_get_option( $prefix, 'demo_box_model', null );
```

### From Metabox

```php
$value = kavro_get_post_meta( get_the_ID(), $metabox_prefix, 'demo_box_model', null );
```
...
```php
$value = get_theme_mod( 'demo_box_model', null );
```

### From Taxonomy Options

```php
$value = kavro_get_term_meta( $term_id, $taxonomy_prefix, 'demo_box_model', null );
```

### From Profile/User Options

```php
$value = kavro_get_user_meta( $user_id, $profile_prefix, 'demo_box_model', null );
```

### From Nav Menu Options

```php
$value = kavro_get_nav_menu_item_meta( $menu_item_id, $nav_menu_prefix, 'demo_box_model', null );
```

### From Widget Options

```php
$value = kavro_get_widget_option( $widget_id, $widget_prefix, 'demo_box_model', null );
```

### From Comment Options

```php
$value = kavro_get_comment_meta( $comment_id, $comment_prefix, 'demo_box_model', null );
```
...
```php
$value = isset( $atts['demo_box_model'] ) ? $atts['demo_box_model'] : null;
```
