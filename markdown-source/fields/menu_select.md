...
**Field type:** `menu_select`  
**Renderer class:** `WPSystem`  
**Aliases using the same renderer:** `role_select`, `sidebar_select`, `template_select`, `user_select`
...
KAVRO::createSection( $prefix, array(
    'title'  => 'Menu Select Example',
    'fields' => array(
        array(
            'id' => 'demo_menu_select',
            'type' => 'menu_select',
            'title' => 'Menu Select',
            'subtitle' => 'Menu Select field example.',
            'desc' => 'This is a documented menu_select field example.',
            'source' => 'menus',
...
KAVRO::createSection( $nav_menu_prefix, array(
    'title'  => 'Menu Select Menu Item Field',
    'fields' => array(
        array(
            'id'       => 'demo_menu_select',
            'type'     => 'menu_select',
...
KAVRO::createSection( $widget_prefix, array(
    'title'  => 'Menu Select Widget Field',
    'fields' => array(
        array(
            'id'       => 'demo_menu_select',
            'type'     => 'menu_select',
...
KAVRO::createSection( $comment_prefix, array(
    'title'  => 'Menu Select Comment Field',
    'fields' => array(
        array(
            'id'       => 'demo_menu_select',
            'type'     => 'menu_select',
...
```php
$value = kavro_get_option( $prefix, 'demo_menu_select', null );
```

### From Metabox

```php
$value = kavro_get_post_meta( get_the_ID(), $metabox_prefix, 'demo_menu_select', null );
```
...
```php
$value = get_theme_mod( 'demo_menu_select', null );
```

### From Taxonomy Options

```php
$value = kavro_get_term_meta( $term_id, $taxonomy_prefix, 'demo_menu_select', null );
```

### From Profile/User Options

```php
$value = kavro_get_user_meta( $user_id, $profile_prefix, 'demo_menu_select', null );
```

### From Nav Menu Options

```php
$value = kavro_get_nav_menu_item_meta( $menu_item_id, $nav_menu_prefix, 'demo_menu_select', null );
```

### From Widget Options

```php
$value = kavro_get_widget_option( $widget_id, $widget_prefix, 'demo_menu_select', null );
```

### From Comment Options

```php
$value = kavro_get_comment_meta( $comment_id, $comment_prefix, 'demo_menu_select', null );
```
...
```php
$value = isset( $atts['demo_menu_select'] ) ? $atts['demo_menu_select'] : null;
```
