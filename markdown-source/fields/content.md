...
**Field type:** `content`  
**Renderer class:** `Content`  
**Aliases using the same renderer:** None

## Purpose

Static content block inside a settings page.
...
KAVRO::createSection( $prefix, array(
    ...
    'fields' => array(
        array(
            'type' => 'content',
            'title' => 'Content',
            'subtitle' => 'Content field example.',
            'desc' => 'This is a documented content field example.',
...
```php
$value = get_theme_mod( 'demo_content', null );
```

### From Taxonomy Options

```php
$value = kavro_get_term_meta( $term_id, $taxonomy_prefix, 'demo_content', null );
```

### From Profile/User Options

```php
$value = kavro_get_user_meta( $user_id, $profile_prefix, 'demo_content', null );
```

### From Nav Menu Options

```php
$value = kavro_get_nav_menu_item_meta( $menu_item_id, $nav_menu_prefix, 'demo_content', null );
```

### From Widget Options

```php
$value = kavro_get_widget_option( $widget_id, $widget_prefix, 'demo_content', null );
```

### From Comment Options

```php
$value = kavro_get_comment_meta( $comment_id, $comment_prefix, 'demo_content', null );
```
...
```php
$value = isset( $atts['demo_content'] ) ? $atts['demo_content'] : null;
```
