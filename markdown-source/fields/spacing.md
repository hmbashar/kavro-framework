...
**Field type:** `spacing`  
**Renderer class:** `Spacing`  
**Aliases using the same renderer:** None

## Purpose

Top/right/bottom/left spacing control.
...
KAVRO::createSection( $prefix, array(
    ...
    'fields' => array(
        array(
            ...
            'type' => 'spacing',
            'title' => 'Spacing',
            'subtitle' => 'Spacing field example.',
            'desc' => 'This is a documented spacing field example.',
            'default' => array(
                'top' => '20',
                'right' => '20',
                'bottom' => '20',
...
```php
$value = kavro_get_option( $prefix, 'demo_spacing', null );
```

### From Metabox

```php
$value = kavro_get_post_meta( get_the_ID(), $metabox_prefix, 'demo_spacing', null );
```
...
```php
$value = get_theme_mod( 'demo_spacing', null );
```

### From Taxonomy Options

```php
$value = kavro_get_term_meta( $term_id, $taxonomy_prefix, 'demo_spacing', null );
```

### From Profile/User Options

```php
$value = kavro_get_user_meta( $user_id, $profile_prefix, 'demo_spacing', null );
```

### From Nav Menu Options

```php
$value = kavro_get_nav_menu_item_meta( $menu_item_id, $nav_menu_prefix, 'demo_spacing', null );
```

### From Widget Options

```php
$value = kavro_get_widget_option( $widget_id, $widget_prefix, 'demo_spacing', null );
```

### From Comment Options

```php
$value = kavro_get_comment_meta( $comment_id, $comment_prefix, 'demo_spacing', null );
```
...
```php
$value = isset( $atts['demo_spacing'] ) ? $atts['demo_spacing'] : null;
```
