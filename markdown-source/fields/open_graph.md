...
**Field type:** `open_graph`  
**Renderer class:** `OpenGraph`  
**Aliases using the same renderer:** None

## Purpose

Open Graph social metadata group.
...
KAVRO::createSection( $prefix, array(
    ...
    'fields' => array(
        array(
            ...
            'type' => 'open_graph',
            'title' => 'Open Graph',
            'subtitle' => 'Open Graph field example.',
            'desc' => 'This is a documented open_graph field example.',
...
KAVRO::createSection( $nav_menu_prefix, array(
    'title'  => 'Open Graph Menu Item Field',
    'fields' => array(
        array(
            'id'       => 'demo_open_graph',
            'type'     => 'open_graph',
...
KAVRO::createSection( $comment_prefix, array(
    'title'  => 'Open Graph Comment Field',
    'fields' => array(
        array(
            'id'       => 'demo_open_graph',
            'type'     => 'open_graph',
...
```php
$value = kavro_get_option( $prefix, 'demo_open_graph', null );
```

### From Metabox

```php
$value = kavro_get_post_meta( get_the_ID(), $metabox_prefix, 'demo_open_graph', null );
```
...
```php
$value = get_theme_mod( 'demo_open_graph', null );
```

### From Taxonomy Options

```php
$value = kavro_get_term_meta( $term_id, $taxonomy_prefix, 'demo_open_graph', null );
```

### From Profile/User Options

```php
$value = kavro_get_user_meta( $user_id, $profile_prefix, 'demo_open_graph', null );
```

### From Nav Menu Options

```php
$value = kavro_get_nav_menu_item_meta( $menu_item_id, $nav_menu_prefix, 'demo_open_graph', null );
```

### From Widget Options

```php
$value = kavro_get_widget_option( $widget_id, $widget_prefix, 'demo_open_graph', null );
```

### From Comment Options

```php
$value = kavro_get_comment_meta( $comment_id, $comment_prefix, 'demo_open_graph', null );
```
...
```php
$value = isset( $atts['demo_open_graph'] ) ? $atts['demo_open_graph'] : null;
```
