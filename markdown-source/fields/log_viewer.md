...
**Field type:** `log_viewer`  
**Renderer class:** `LogViewer`  
**Aliases using the same renderer:** None

## Purpose

Log viewer/display field.
...
| Attribute | Type | Description |
|---|---:|---|
| `logs` | array | Log entries or log source metadata. |
...
KAVRO::createSection( $prefix, array(
    ...
    'fields' => array(
        array(
            ...
            'type' => 'log_viewer',
            'title' => 'Log Viewer',
            'subtitle' => 'Log Viewer field example.',
            'desc' => 'This is a documented log_viewer field example.',
        ),
    ),
) );
```
...
KAVRO::createSection( $nav_menu_prefix, array(
    'title'  => 'Log Viewer Menu Item Field',
    'fields' => array(
        array(
            'id'       => 'demo_log_viewer',
            'type'     => 'log_viewer',
...
KAVRO::createSection( $comment_prefix, array(
    'title'  => 'Log Viewer Comment Field',
    'fields' => array(
        array(
            'id'       => 'demo_log_viewer',
            'type'     => 'log_viewer',
...
```php
$value = kavro_get_option( $prefix, 'demo_log_viewer', null );
```

### From Metabox

```php
$value = kavro_get_post_meta( get_the_ID(), $metabox_prefix, 'demo_log_viewer', null );
```
...
```php
$value = get_theme_mod( 'demo_log_viewer', null );
```

### From Taxonomy Options

```php
$value = kavro_get_term_meta( $term_id, $taxonomy_prefix, 'demo_log_viewer', null );
```

### From Profile/User Options

```php
$value = kavro_get_user_meta( $user_id, $profile_prefix, 'demo_log_viewer', null );
```

### From Nav Menu Options

```php
$value = kavro_get_nav_menu_item_meta( $menu_item_id, $nav_menu_prefix, 'demo_log_viewer', null );
```

### From Widget Options

```php
$value = kavro_get_widget_option( $widget_id, $widget_prefix, 'demo_log_viewer', null );
```

### From Comment Options

```php
$value = kavro_get_comment_meta( $comment_id, $comment_prefix, 'demo_log_viewer', null );
```
...
```php
$value = isset( $atts['demo_log_viewer'] ) ? $atts['demo_log_viewer'] : null;
```
