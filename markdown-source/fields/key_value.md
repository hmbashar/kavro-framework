...
KAVRO::createSection( $prefix, array(
    ...
    'fields' => array(
        array(
            ...
            'type' => 'key_value',
            'title' => 'Key Value',
            'subtitle' => 'Key Value field example.',
            'desc' => 'This is a documented key_value field example.',
...
KAVRO::createSection( $nav_menu_prefix, array(
    'title'  => 'Key Value Menu Item Field',
    'fields' => array(
        array(
            'id'       => 'demo_key_value',
            'type'     => 'key_value',
...
```php
$value = kavro_get_option( $prefix, 'demo_key_value', null );
```

### From Metabox

```php
$value = kavro_get_post_meta( get_the_ID(), $metabox_prefix, 'demo_key_value', null );
```
...
```php
$value = get_theme_mod( 'demo_key_value', null );
```

### From Taxonomy Options

```php
$value = kavro_get_term_meta( $term_id, $taxonomy_prefix, 'demo_key_value', null );
```

### From Profile/User Options

```php
$value = kavro_get_user_meta( $user_id, $profile_prefix, 'demo_key_value', null );
```

### From Nav Menu Options

```php
$value = kavro_get_nav_menu_item_meta( $menu_item_id, $nav_menu_prefix, 'demo_key_value', null );
```

### From Widget Options

```php
$value = kavro_get_widget_option( $widget_id, $widget_prefix, 'demo_key_value', null );
```

### From Comment Options

```php
$value = kavro_get_comment_meta( $comment_id, $comment_prefix, 'demo_key_value', null );
```
...
```php
$value = isset( $atts['demo_key_value'] ) ? $atts['demo_key_value'] : null;
```
