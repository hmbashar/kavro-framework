...
**Aliases using the same renderer:** `slider`
...
KAVRO::createSection( $prefix, array(
    ...
    'fields' => array(
        array(
            ...
            'type' => 'range',
            'title' => 'Range',
            'subtitle' => 'Range field example.',
            'desc' => 'This is a documented range field example.',
            'min' => 0,
            'max' => 100,
            'step' => 1,
            'default' => 50,
...
```php
$value = kavro_get_option( $prefix, 'demo_range', null );
```

### From Metabox

```php
$value = kavro_get_post_meta( get_the_ID(), $metabox_prefix, 'demo_range', null );
```
...
```php
$value = get_theme_mod( 'demo_range', null );
```

### From Taxonomy Options

```php
$value = kavro_get_term_meta( $term_id, $taxonomy_prefix, 'demo_range', null );
```

### From Profile/User Options

```php
$value = kavro_get_user_meta( $user_id, $profile_prefix, 'demo_range', null );
```

### From Nav Menu Options

```php
$value = kavro_get_nav_menu_item_meta( $menu_item_id, $nav_menu_prefix, 'demo_range', null );
```

### From Widget Options

```php
$value = kavro_get_widget_option( $widget_id, $widget_prefix, 'demo_range', null );
```

### From Comment Options

```php
$value = kavro_get_comment_meta( $comment_id, $comment_prefix, 'demo_range', null );
```
...
```php
$value = isset( $atts['demo_range'] ) ? $atts['demo_range'] : null;
```
