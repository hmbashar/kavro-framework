...
**Renderer class:** `FileUpload`  
**Aliases using the same renderer:** `audio_upload`, `file_upload`
...
KAVRO::createSection( $prefix, array(
    'title'  => 'Video Upload Example',
    'fields' => array(
        array(
            'id' => 'demo_video_upload',
            'type' => 'video_upload',
            'title' => 'Video Upload',
            'subtitle' => 'Video Upload field example.',
            'desc' => 'This is a documented video_upload field example.',
            'library' => 'all',
...
KAVRO::createSection( $nav_menu_prefix, array(
    'title'  => 'Video Upload Menu Item Field',
    'fields' => array(
        array(
            'id'       => 'demo_video_upload',
            'type'     => 'video_upload',
...
KAVRO::createSection( $widget_prefix, array(
    'title'  => 'Video Upload Widget Field',
    'fields' => array(
        array(
            'id'       => 'demo_video_upload',
            'type'     => 'video_upload',
...
KAVRO::createSection( $comment_prefix, array(
    'title'  => 'Video Upload Comment Field',
    'fields' => array(
        array(
            'id'       => 'demo_video_upload',
            'type'     => 'video_upload',
...
```php
$value = kavro_get_option( $prefix, 'demo_video_upload', null );
```

### From Metabox

```php
$value = kavro_get_post_meta( get_the_ID(), $metabox_prefix, 'demo_video_upload', null );
```
...
```php
$value = get_theme_mod( 'demo_video_upload', null );
```

### From Taxonomy Options

```php
$value = kavro_get_term_meta( $term_id, $taxonomy_prefix, 'demo_video_upload', null );
```

### From Profile/User Options

```php
$value = kavro_get_user_meta( $user_id, $profile_prefix, 'demo_video_upload', null );
```

### From Nav Menu Options

```php
$value = kavro_get_nav_menu_item_meta( $menu_item_id, $nav_menu_prefix, 'demo_video_upload', null );
```

### From Widget Options

```php
$value = kavro_get_widget_option( $widget_id, $widget_prefix, 'demo_video_upload', null );
```

### From Comment Options

```php
$value = kavro_get_comment_meta( $comment_id, $comment_prefix, 'demo_video_upload', null );
```
...
```php
$value = isset( $atts['demo_video_upload'] ) ? $atts['demo_video_upload'] : null;
```
