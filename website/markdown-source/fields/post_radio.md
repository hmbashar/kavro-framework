...
**Field type:** `post_radio`  
**Renderer class:** `WPContent`  
**Aliases using the same renderer:** `cpt_select`, `page_select`, `post_autocomplete`, `post_checkbox`, `post_relation`, `post_select`
...
KAVRO::createSection( $prefix, array(
    'title'  => 'Post Radio Example',
    'fields' => array(
        array(
            'id' => 'demo_post_radio',
            'type' => 'post_radio',
            'title' => 'Post Radio',
            'subtitle' => 'Post Radio field example.',
            'desc' => 'This is a documented post_radio field example.',
            'post_type' => 'post',
            'post_status' => 'publish',
            'limit' => 20,
...
KAVRO::createSection( $nav_menu_prefix, array(
    'title'  => 'Post Radio Menu Item Field',
    'fields' => array(
        array(
            'id'       => 'demo_post_radio',
            'type'     => 'post_radio',
...
KAVRO::createSection( $comment_prefix, array(
    'title'  => 'Post Radio Comment Field',
    'fields' => array(
        array(
            'id'       => 'demo_post_radio',
            'type'     => 'post_radio',
...
```php
$value = kavro_get_option( $prefix, 'demo_post_radio', null );
```

### From Metabox

```php
$value = kavro_get_post_meta( get_the_ID(), $metabox_prefix, 'demo_post_radio', null );
```
...
```php
$value = get_theme_mod( 'demo_post_radio', null );
```

### From Taxonomy Options

```php
$value = kavro_get_term_meta( $term_id, $taxonomy_prefix, 'demo_post_radio', null );
```

### From Profile/User Options

```php
$value = kavro_get_user_meta( $user_id, $profile_prefix, 'demo_post_radio', null );
```

### From Nav Menu Options

```php
$value = kavro_get_nav_menu_item_meta( $menu_item_id, $nav_menu_prefix, 'demo_post_radio', null );
```

### From Widget Options

```php
$value = kavro_get_widget_option( $widget_id, $widget_prefix, 'demo_post_radio', null );
```

### From Comment Options

```php
$value = kavro_get_comment_meta( $comment_id, $comment_prefix, 'demo_post_radio', null );
```
...
```php
$value = isset( $atts['demo_post_radio'] ) ? $atts['demo_post_radio'] : null;
```
