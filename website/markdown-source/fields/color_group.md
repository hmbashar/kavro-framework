...
**Field type:** `color_group`  
**Renderer class:** `ColorGroup`  
...
KAVRO::createSection( $prefix, array(
    'title'  => 'Color Group Example',
    'fields' => array(
        array(
            'id' => 'demo_color_group',
            'type' => 'color_group',
            'title' => 'Color Group',
            'subtitle' => 'Color Group field example.',
            'desc' => 'This is a documented color_group field example.',
            'options' => array(
                'one' => 'One',
                'two' => 'Two',
                'three' => 'Three',
...
KAVRO::createSection( $nav_menu_prefix, array(
    'title'  => 'Color Group Menu Item Field',
    'fields' => array(
        array(
            'id'       => 'demo_color_group',
            'type'     => 'color_group',
...
KAVRO::createSection( $widget_prefix, array(
    'title'  => 'Color Group Widget Field',
    'fields' => array(
        array(
            'id'       => 'demo_color_group',
            'type'     => 'color_group',
...
KAVRO::createSection( $comment_prefix, array(
    'title'  => 'Color Group Comment Field',
    'fields' => array(
        array(
            'id'       => 'demo_color_group',
            'type'     => 'color_group',
...
```php
$value = kavro_get_option( $prefix, 'demo_color_group', null );
```

### From Metabox

```php
$value = kavro_get_post_meta( get_the_ID(), $metabox_prefix, 'demo_color_group', null );
```
...
```php
$value = get_theme_mod( 'demo_color_group', null );
```

### From Taxonomy Options

```php
$value = kavro_get_term_meta( $term_id, $taxonomy_prefix, 'demo_color_group', null );
```

### From Profile/User Options

```php
$value = kavro_get_user_meta( $user_id, $profile_prefix, 'demo_color_group', null );
```

### From Nav Menu Options

```php
$value = kavro_get_nav_menu_item_meta( $menu_item_id, $nav_menu_prefix, 'demo_color_group', null );
```

### From Widget Options

```php
$value = kavro_get_widget_option( $widget_id, $widget_prefix, 'demo_color_group', null );
```

### From Comment Options

```php
$value = kavro_get_comment_meta( $comment_id, $comment_prefix, 'demo_color_group', null );
```
...
```php
$value = isset( $atts['demo_color_group'] ) ? $atts['demo_color_group'] : null;
```
