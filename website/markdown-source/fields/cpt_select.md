...
**Field type:** `cpt_select`  
**Renderer class:** `WPContent`  
**Aliases using the same renderer:** `page_select`, `post_autocomplete`, `post_checkbox`, `post_radio`, `post_relation`, `post_select`
...
KAVRO::createSection( $prefix, array(
    'title'  => 'Cpt Select Example',
    'fields' => array(
        array(
            'id' => 'demo_cpt_select',
            'type' => 'cpt_select',
            'title' => 'Cpt Select',
            'subtitle' => 'Cpt Select field example.',
            'desc' => 'This is a documented cpt_select field example.',
            'post_type' => array(
                'post',
                'page',
            ),
...
KAVRO::createSection( $nav_menu_prefix, array(
    'title'  => 'Cpt Select Menu Item Field',
    'fields' => array(
        array(
            'id'       => 'demo_cpt_select',
            'type'     => 'cpt_select',
...
KAVRO::createSection( $comment_prefix, array(
    'title'  => 'Cpt Select Comment Field',
    'fields' => array(
        array(
            'id'       => 'demo_cpt_select',
            'type'     => 'cpt_select',
...
```php
$value = kavro_get_option( $prefix, 'demo_cpt_select', null );
```

### From Metabox

```php
$value = kavro_get_post_meta( get_the_ID(), $metabox_prefix, 'demo_cpt_select', null );
```
...
```php
$value = get_theme_mod( 'demo_cpt_select', null );
```

### From Taxonomy Options

```php
$value = kavro_get_term_meta( $term_id, $taxonomy_prefix, 'demo_cpt_select', null );
```

### From Profile/User Options

```php
$value = kavro_get_user_meta( $user_id, $profile_prefix, 'demo_cpt_select', null );
```

### From Nav Menu Options

```php
$value = kavro_get_nav_menu_item_meta( $menu_item_id, $nav_menu_prefix, 'demo_cpt_select', null );
```

### From Widget Options

```php
$value = kavro_get_widget_option( $widget_id, $widget_prefix, 'demo_cpt_select', null );
```

### From Comment Options

```php
$value = kavro_get_comment_meta( $comment_id, $comment_prefix, 'demo_cpt_select', null );
```
...
```php
$value = isset( $atts['demo_cpt_select'] ) ? $atts['demo_cpt_select'] : null;
```
