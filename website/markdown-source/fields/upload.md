...
**Field type:** `upload`  
**Renderer class:** `Upload`  
**Aliases using the same renderer:** None

## Purpose

Upload/media URL field.
...
KAVRO::createSection( $prefix, array(
    ...
    'fields' => array(
        array(
            ...
            'type' => 'upload',
            'title' => 'Upload',
            'subtitle' => 'Upload field example.',
            'desc' => 'This is a documented upload field example.',
            'library' => 'all',
...
```php
$value = kavro_get_option( $prefix, 'demo_upload', null );
```

### From Metabox

```php
$value = kavro_get_post_meta( get_the_ID(), $metabox_prefix, 'demo_upload', null );
```
...
```php
$value = get_theme_mod( 'demo_upload', null );
```

### From Taxonomy Options

```php
$value = kavro_get_term_meta( $term_id, $taxonomy_prefix, 'demo_upload', null );
```

### From Profile/User Options

```php
$value = kavro_get_user_meta( $user_id, $profile_prefix, 'demo_upload', null );
```

### From Nav Menu Options

```php
$value = kavro_get_nav_menu_item_meta( $menu_item_id, $nav_menu_prefix, 'demo_upload', null );
```

### From Widget Options

```php
$value = kavro_get_widget_option( $widget_id, $widget_prefix, 'demo_upload', null );
```

### From Comment Options

```php
$value = kavro_get_comment_meta( $comment_id, $comment_prefix, 'demo_upload', null );
```
...
```php
$value = isset( $atts['demo_upload'] ) ? $atts['demo_upload'] : null;
```
