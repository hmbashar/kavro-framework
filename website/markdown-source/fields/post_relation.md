...
**Field type:** `post_relation`  
**Renderer class:** `WPContent`  
**Aliases using the same renderer:** `cpt_select`, `page_select`, `post_autocomplete`, `post_checkbox`, `post_radio`, `post_select`

## Purpose

Multiple post relationship selector.
...
KAVRO::createSection( $prefix, array(
    'title'  => 'Post Relation Example',
    'fields' => array(
        array(
            'id' => 'demo_post_relation',
            'type' => 'post_relation',
            'title' => 'Post Relation',
            'subtitle' => 'Post Relation field example.',
            'desc' => 'This is a documented post_relation field example.',
            'post_type' => 'post',
            'post_status' => 'publish',
            'limit' => 20,
            'default' => array(
...
KAVRO::createSection( $customize_prefix, array(
    'title'  => 'Post Relation Customizer',
    'fields' => array(
        array(
            'id'       => 'demo_post_relation',
            'type'     => 'post_relation',
...
KAVRO::createSection( $taxonomy_prefix, array(
    'title'  => 'Post Relation Term Field',
    'fields' => array(
        array(
            'id'       => 'demo_post_relation',
            'type'     => 'post_relation',
...
KAVRO::createSection( $profile_prefix, array(
    'title'  => 'Post Relation User Field',
    'fields' => array(
        array(
            'id'       => 'demo_post_relation',
            'type'     => 'post_relation',
...
KAVRO::createSection( $nav_menu_prefix, array(
    'title'  => 'Post Relation Menu Item Field',
    'fields' => array(
        array(
            'id'       => 'demo_post_relation',
            'type'     => 'post_relation',
...
KAVRO::createSection( $widget_prefix, array(
    'title'  => 'Post Relation Widget Field',
    'fields' => array(
        array(
            'id'       => 'demo_post_relation',
            'type'     => 'post_relation',
...
KAVRO::createSection( $comment_prefix, array(
    'title'  => 'Post Relation Comment Field',
    'fields' => array(
        array(
            'id'       => 'demo_post_relation',
            'type'     => 'post_relation',
...
```php
$value = kavro_get_option( $prefix, 'demo_post_relation', null );
```

### From Metabox

```php
$value = kavro_get_post_meta( get_the_ID(), $metabox_prefix, 'demo_post_relation', null );
```
...
```php
$value = get_theme_mod( 'demo_post_relation', null );
```

### From Taxonomy Options

```php
$value = kavro_get_term_meta( $term_id, $taxonomy_prefix, 'demo_post_relation', null );
```

### From Profile/User Options

```php
$value = kavro_get_user_meta( $user_id, $profile_prefix, 'demo_post_relation', null );
```

### From Nav Menu Options

```php
$value = kavro_get_nav_menu_item_meta( $menu_item_id, $nav_menu_prefix, 'demo_post_relation', null );
```

### From Widget Options

```php
$value = kavro_get_widget_option( $widget_id, $widget_prefix, 'demo_post_relation', null );
```

### From Comment Options

```php
$value = kavro_get_comment_meta( $comment_id, $comment_prefix, 'demo_post_relation', null );
```
...
```php
$value = isset( $atts['demo_post_relation'] ) ? $atts['demo_post_relation'] : null;
```
