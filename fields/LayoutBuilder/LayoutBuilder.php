...
class LayoutBuilder extends AbstractField {
    ...
    public function render() {
        ...
        foreach ( $layouts as $key => $label ) {
            echo '<label class="kavro-layout-choice"><input type="radio" name="' . esc_attr( $this->name ) . '" value="' . esc_attr( $key ) . '" ' . checked( $this->value, $key, false ) . '><span>' . esc_html( $label ) . '</span></label>';
        }
        echo '</div>';
    }
}
