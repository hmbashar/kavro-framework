...
KAVRO::createSection( $prefix, array(
    ...
    'fields' => array(
        array(
            ...
            'type' => 'datetime',
            'title' => 'Datetime',
            'subtitle' => 'Datetime field example.',
            'desc' => 'This is a documented datetime field example.',
            'default' => '2026-01-01 10:30',
        ),
    ),
) );
```
...
KAVRO::createSection( $nav_menu_prefix, array(
    'title'  => 'Datetime Menu Item Field',
    'fields' => array(
        array(
            'id'       => 'demo_datetime',
            'type'     => 'datetime',
...
```php
$value = kavro_get_option( $prefix, 'demo_datetime', null );
```

### From Metabox

```php
$value = kavro_get_post_meta( get_the_ID(), $metabox_prefix, 'demo_datetime', null );
```
...
```php
$value = get_theme_mod( 'demo_datetime', null );
```

### From Taxonomy Options

```php
$value = kavro_get_term_meta( $term_id, $taxonomy_prefix, 'demo_datetime', null );
```

### From Profile/User Options

```php
$value = kavro_get_user_meta( $user_id, $profile_prefix, 'demo_datetime', null );
```

### From Nav Menu Options

```php
$value = kavro_get_nav_menu_item_meta( $menu_item_id, $nav_menu_prefix, 'demo_datetime', null );
```

### From Widget Options

```php
$value = kavro_get_widget_option( $widget_id, $widget_prefix, 'demo_datetime', null );
```

### From Comment Options

```php
$value = kavro_get_comment_meta( $comment_id, $comment_prefix, 'demo_datetime', null );
```
...
```php
$value = isset( $atts['demo_datetime'] ) ? $atts['demo_datetime'] : null;
```
