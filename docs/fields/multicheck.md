...
KAVRO::createSection( $nav_menu_prefix, array(
    'title'  => 'Multicheck Menu Item Field',
    'fields' => array(
        array(
            'id'       => 'demo_multicheck',
            'type'     => 'multicheck',
...
KAVRO::createSection( $comment_prefix, array(
    'title'  => 'Multicheck Comment Field',
    'fields' => array(
        array(
            'id'       => 'demo_multicheck',
            'type'     => 'multicheck',
...
```php
$value = get_theme_mod( 'demo_multicheck', null );
```

### From Taxonomy Options

```php
$value = kavro_get_term_meta( $term_id, $taxonomy_prefix, 'demo_multicheck', null );
```

### From Profile/User Options

```php
$value = kavro_get_user_meta( $user_id, $profile_prefix, 'demo_multicheck', null );
```

### From Nav Menu Options

```php
$value = kavro_get_nav_menu_item_meta( $menu_item_id, $nav_menu_prefix, 'demo_multicheck', null );
```

### From Widget Options

```php
$value = kavro_get_widget_option( $widget_id, $widget_prefix, 'demo_multicheck', null );
```

### From Comment Options

```php
$value = kavro_get_comment_meta( $comment_id, $comment_prefix, 'demo_multicheck', null );
```
...
```php
$value = isset( $atts['demo_multicheck'] ) ? $atts['demo_multicheck'] : null;
```
