...
KAVRO::createSection( $prefix, array(
    ...
    'fields' => array(
        array(
            ...
            'type' => 'time',
            'title' => 'Time',
            'subtitle' => 'Time field example.',
            'desc' => 'This is a documented time field example.',
            'default' => '10:30',
        ),
    ),
) );
...
```php
$value = kavro_get_option( $prefix, 'demo_time', null );
```

### From Metabox

```php
$value = kavro_get_post_meta( get_the_ID(), $metabox_prefix, 'demo_time', null );
```
...
```php
$value = get_theme_mod( 'demo_time', null );
```

### From Taxonomy Options

```php
$value = kavro_get_term_meta( $term_id, $taxonomy_prefix, 'demo_time', null );
```

### From Profile/User Options

```php
$value = kavro_get_user_meta( $user_id, $profile_prefix, 'demo_time', null );
```

### From Nav Menu Options

```php
$value = kavro_get_nav_menu_item_meta( $menu_item_id, $nav_menu_prefix, 'demo_time', null );
```

### From Widget Options

```php
$value = kavro_get_widget_option( $widget_id, $widget_prefix, 'demo_time', null );
```

### From Comment Options

```php
$value = kavro_get_comment_meta( $comment_id, $comment_prefix, 'demo_time', null );
```
...
```php
$value = isset( $atts['demo_time'] ) ? $atts['demo_time'] : null;
```
