...
KAVRO::createSection( $prefix, array(
    ...
    'fields' => array(
        array(
            ...
            'type' => 'border',
            'title' => 'Border',
            'subtitle' => 'Border field example.',
            'desc' => 'This is a documented border field example.',
            'default' => array(
                'width' => '1',
                'style' => 'solid',
                'color' => '#e5e7eb',
...
```php
$value = kavro_get_option( $prefix, 'demo_border', null );
```

### From Metabox

```php
$value = kavro_get_post_meta( get_the_ID(), $metabox_prefix, 'demo_border', null );
```
...
```php
$value = get_theme_mod( 'demo_border', null );
```

### From Taxonomy Options

```php
$value = kavro_get_term_meta( $term_id, $taxonomy_prefix, 'demo_border', null );
```

### From Profile/User Options

```php
$value = kavro_get_user_meta( $user_id, $profile_prefix, 'demo_border', null );
```

### From Nav Menu Options

```php
$value = kavro_get_nav_menu_item_meta( $menu_item_id, $nav_menu_prefix, 'demo_border', null );
```

### From Widget Options

```php
$value = kavro_get_widget_option( $widget_id, $widget_prefix, 'demo_border', null );
```

### From Comment Options

```php
$value = kavro_get_comment_meta( $comment_id, $comment_prefix, 'demo_border', null );
```
...
```php
$value = isset( $atts['demo_border'] ) ? $atts['demo_border'] : null;
```
