...
| Attribute | Type | Description |
|---|---:|---|
| `tabs` | array | Tabbed group definitions. Each tab can contain a title and nested fields/content. |

## Options Framework example

```php
KAVRO::createSection( $prefix, array(
    'title'  => 'Tabbed Example',
    'fields' => array(
        array(
            'id' => 'demo_tabbed',
            'type' => 'tabbed',
            'title' => 'Tabbed',
            'subtitle' => 'Tabbed field example.',
            'desc' => 'This is a documented tabbed field example.',
            'tabs' => array(
                array(
                    'title' => 'Content',
                    'fields' => array(
...
```php
$value = kavro_get_option( $prefix, 'demo_tabbed', null );
```

### From Metabox

```php
$value = kavro_get_post_meta( get_the_ID(), $metabox_prefix, 'demo_tabbed', null );
```
...
```php
$value = get_theme_mod( 'demo_tabbed', null );
```

### From Taxonomy Options

```php
$value = kavro_get_term_meta( $term_id, $taxonomy_prefix, 'demo_tabbed', null );
```

### From Profile/User Options

```php
$value = kavro_get_user_meta( $user_id, $profile_prefix, 'demo_tabbed', null );
```

### From Nav Menu Options

```php
$value = kavro_get_nav_menu_item_meta( $menu_item_id, $nav_menu_prefix, 'demo_tabbed', null );
```

### From Widget Options

```php
$value = kavro_get_widget_option( $widget_id, $widget_prefix, 'demo_tabbed', null );
```

### From Comment Options

```php
$value = kavro_get_comment_meta( $comment_id, $comment_prefix, 'demo_tabbed', null );
```
...
```php
$value = isset( $atts['demo_tabbed'] ) ? $atts['demo_tabbed'] : null;
```
