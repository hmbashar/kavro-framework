...
**Aliases using the same renderer:** `audio_upload`, `video_upload`
...
KAVRO::createSection( $prefix, array(
    ...
    'fields' => array(
        array(
            ...
            'type' => 'file_upload',
            'title' => 'File Upload',
            'subtitle' => 'File Upload field example.',
            'desc' => 'This is a documented file_upload field example.',
            'library' => 'all',
...
KAVRO::createSection( $nav_menu_prefix, array(
    'title'  => 'File Upload Menu Item Field',
    'fields' => array(
        array(
            'id'       => 'demo_file_upload',
            'type'     => 'file_upload',
...
KAVRO::createSection( $widget_prefix, array(
    'title'  => 'File Upload Widget Field',
    'fields' => array(
        array(
            'id'       => 'demo_file_upload',
            'type'     => 'file_upload',
...
KAVRO::createSection( $comment_prefix, array(
    'title'  => 'File Upload Comment Field',
    'fields' => array(
        array(
            'id'       => 'demo_file_upload',
            'type'     => 'file_upload',
...
```php
$value = kavro_get_option( $prefix, 'demo_file_upload', null );
```

### From Metabox

```php
$value = kavro_get_post_meta( get_the_ID(), $metabox_prefix, 'demo_file_upload', null );
```
...
```php
$value = get_theme_mod( 'demo_file_upload', null );
```

### From Taxonomy Options

```php
$value = kavro_get_term_meta( $term_id, $taxonomy_prefix, 'demo_file_upload', null );
```

### From Profile/User Options

```php
$value = kavro_get_user_meta( $user_id, $profile_prefix, 'demo_file_upload', null );
```

### From Nav Menu Options

```php
$value = kavro_get_nav_menu_item_meta( $menu_item_id, $nav_menu_prefix, 'demo_file_upload', null );
```

### From Widget Options

```php
$value = kavro_get_widget_option( $widget_id, $widget_prefix, 'demo_file_upload', null );
```

### From Comment Options

```php
$value = kavro_get_comment_meta( $comment_id, $comment_prefix, 'demo_file_upload', null );
```
...
```php
$value = isset( $atts['demo_file_upload'] ) ? $atts['demo_file_upload'] : null;
```
