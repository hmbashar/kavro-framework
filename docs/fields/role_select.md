...
**Aliases using the same renderer:** `menu_select`, `sidebar_select`, `template_select`, `user_select`
...
KAVRO::createSection( $prefix, array(
    'title'  => 'Role Select Example',
    'fields' => array(
        array(
            'id' => 'demo_role_select',
            'type' => 'role_select',
            'title' => 'Role Select',
            'subtitle' => 'Role Select field example.',
            'desc' => 'This is a documented role_select field example.',
            'source' => 'roles',
...
KAVRO::createSection( $nav_menu_prefix, array(
    'title'  => 'Role Select Menu Item Field',
    'fields' => array(
        array(
            'id'       => 'demo_role_select',
            'type'     => 'role_select',
...
KAVRO::createSection( $widget_prefix, array(
    'title'  => 'Role Select Widget Field',
    'fields' => array(
        array(
            'id'       => 'demo_role_select',
            'type'     => 'role_select',
...
KAVRO::createSection( $comment_prefix, array(
    'title'  => 'Role Select Comment Field',
    'fields' => array(
        array(
            'id'       => 'demo_role_select',
            'type'     => 'role_select',
...
```php
$value = kavro_get_option( $prefix, 'demo_role_select', null );
```

### From Metabox

```php
$value = kavro_get_post_meta( get_the_ID(), $metabox_prefix, 'demo_role_select', null );
```
...
```php
$value = get_theme_mod( 'demo_role_select', null );
```

### From Taxonomy Options

```php
$value = kavro_get_term_meta( $term_id, $taxonomy_prefix, 'demo_role_select', null );
```

### From Profile/User Options

```php
$value = kavro_get_user_meta( $user_id, $profile_prefix, 'demo_role_select', null );
```

### From Nav Menu Options

```php
$value = kavro_get_nav_menu_item_meta( $menu_item_id, $nav_menu_prefix, 'demo_role_select', null );
```

### From Widget Options

```php
$value = kavro_get_widget_option( $widget_id, $widget_prefix, 'demo_role_select', null );
```

### From Comment Options

```php
$value = kavro_get_comment_meta( $comment_id, $comment_prefix, 'demo_role_select', null );
```
...
```php
$value = isset( $atts['demo_role_select'] ) ? $atts['demo_role_select'] : null;
```
