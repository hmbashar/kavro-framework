...
**Field type:** `cron_schedule`  
**Renderer class:** `CronSchedule`  
**Aliases using the same renderer:** None

## Purpose

Cron/schedule configuration field.
...
KAVRO::createSection( $prefix, array(
    ...
    'fields' => array(
        array(
            ...
            'type' => 'cron_schedule',
            'title' => 'Cron Schedule',
            'subtitle' => 'Cron Schedule field example.',
            'desc' => 'This is a documented cron_schedule field example.',
...
KAVRO::createSection( $customize_prefix, array(
    'title'  => 'Cron Schedule Customizer',
    'fields' => array(
        array(
            'id'       => 'demo_cron_schedule',
            'type'     => 'cron_schedule',
...
KAVRO::createSection( $taxonomy_prefix, array(
    'title'  => 'Cron Schedule Term Field',
    'fields' => array(
        array(
            'id'       => 'demo_cron_schedule',
            'type'     => 'cron_schedule',
...
KAVRO::createSection( $profile_prefix, array(
    'title'  => 'Cron Schedule User Field',
    'fields' => array(
        array(
            'id'       => 'demo_cron_schedule',
            'type'     => 'cron_schedule',
...
KAVRO::createSection( $nav_menu_prefix, array(
    'title'  => 'Cron Schedule Menu Item Field',
    'fields' => array(
        array(
            'id'       => 'demo_cron_schedule',
            'type'     => 'cron_schedule',
...
KAVRO::createSection( $widget_prefix, array(
    'title'  => 'Cron Schedule Widget Field',
    'fields' => array(
        array(
            'id'       => 'demo_cron_schedule',
            'type'     => 'cron_schedule',
...
KAVRO::createSection( $comment_prefix, array(
    'title'  => 'Cron Schedule Comment Field',
    'fields' => array(
        array(
            'id'       => 'demo_cron_schedule',
            'type'     => 'cron_schedule',
...
```php
$value = kavro_get_option( $prefix, 'demo_cron_schedule', null );
```

### From Metabox

```php
$value = kavro_get_post_meta( get_the_ID(), $metabox_prefix, 'demo_cron_schedule', null );
```
...
```php
$value = get_theme_mod( 'demo_cron_schedule', null );
```

### From Taxonomy Options

```php
$value = kavro_get_term_meta( $term_id, $taxonomy_prefix, 'demo_cron_schedule', null );
```

### From Profile/User Options

```php
$value = kavro_get_user_meta( $user_id, $profile_prefix, 'demo_cron_schedule', null );
```

### From Nav Menu Options

```php
$value = kavro_get_nav_menu_item_meta( $menu_item_id, $nav_menu_prefix, 'demo_cron_schedule', null );
```

### From Widget Options

```php
$value = kavro_get_widget_option( $widget_id, $widget_prefix, 'demo_cron_schedule', null );
```

### From Comment Options

```php
$value = kavro_get_comment_meta( $comment_id, $comment_prefix, 'demo_cron_schedule', null );
```
...
```php
$value = isset( $atts['demo_cron_schedule'] ) ? $atts['demo_cron_schedule'] : null;
```
