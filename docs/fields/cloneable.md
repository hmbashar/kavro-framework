...
**Field type:** `cloneable`  
**Renderer class:** `Cloneable`  
**Aliases using the same renderer:** None

## Purpose

Cloneable list of sub-fields.
...
KAVRO::createSection( $prefix, array(
    'title'  => 'Cloneable Example',
    'fields' => array(
        array(
            'id' => 'demo_cloneable',
            'type' => 'cloneable',
            'title' => 'Cloneable',
            'subtitle' => 'Cloneable field example.',
            'desc' => 'This is a documented cloneable field example.',
            'fields' => array(
                array(
                    'id' => 'item_title',
                    'type' => 'text',
...
KAVRO::createSection( $nav_menu_prefix, array(
    'title'  => 'Cloneable Menu Item Field',
    'fields' => array(
        array(
            'id'       => 'demo_cloneable',
            'type'     => 'cloneable',
...
```php
$value = kavro_get_option( $prefix, 'demo_cloneable', null );
```

### From Metabox

```php
$value = kavro_get_post_meta( get_the_ID(), $metabox_prefix, 'demo_cloneable', null );
```
...
```php
$value = get_theme_mod( 'demo_cloneable', null );
```

### From Taxonomy Options

```php
$value = kavro_get_term_meta( $term_id, $taxonomy_prefix, 'demo_cloneable', null );
```

### From Profile/User Options

```php
$value = kavro_get_user_meta( $user_id, $profile_prefix, 'demo_cloneable', null );
```

### From Nav Menu Options

```php
$value = kavro_get_nav_menu_item_meta( $menu_item_id, $nav_menu_prefix, 'demo_cloneable', null );
```

### From Widget Options

```php
$value = kavro_get_widget_option( $widget_id, $widget_prefix, 'demo_cloneable', null );
```

### From Comment Options

```php
$value = kavro_get_comment_meta( $comment_id, $comment_prefix, 'demo_cloneable', null );
```
...
```php
$value = isset( $atts['demo_cloneable'] ) ? $atts['demo_cloneable'] : null;
```
