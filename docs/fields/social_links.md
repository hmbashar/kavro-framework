...
KAVRO::createSection( $nav_menu_prefix, array(
    'title'  => 'Social Links Menu Item Field',
    'fields' => array(
        array(
            'id'       => 'demo_social_links',
            'type'     => 'social_links',
...
KAVRO::createSection( $widget_prefix, array(
    'title'  => 'Social Links Widget Field',
    'fields' => array(
        array(
            'id'       => 'demo_social_links',
            'type'     => 'social_links',
...
KAVRO::createSection( $comment_prefix, array(
    'title'  => 'Social Links Comment Field',
    'fields' => array(
        array(
            'id'       => 'demo_social_links',
            'type'     => 'social_links',
...
```php
$value = get_theme_mod( 'demo_social_links', null );
```

### From Taxonomy Options

```php
$value = kavro_get_term_meta( $term_id, $taxonomy_prefix, 'demo_social_links', null );
```

### From Profile/User Options

```php
$value = kavro_get_user_meta( $user_id, $profile_prefix, 'demo_social_links', null );
```

### From Nav Menu Options

```php
$value = kavro_get_nav_menu_item_meta( $menu_item_id, $nav_menu_prefix, 'demo_social_links', null );
```

### From Widget Options

```php
$value = kavro_get_widget_option( $widget_id, $widget_prefix, 'demo_social_links', null );
```

### From Comment Options

```php
$value = kavro_get_comment_meta( $comment_id, $comment_prefix, 'demo_social_links', null );
```
...
```php
$value = isset( $atts['demo_social_links'] ) ? $atts['demo_social_links'] : null;
```
