...
KAVRO::createSection( $nav_menu_prefix, array(
    'title'  => 'User Select Menu Item Field',
    'fields' => array(
        array(
            'id'       => 'demo_user_select',
            'type'     => 'user_select',
...
KAVRO::createSection( $widget_prefix, array(
    'title'  => 'User Select Widget Field',
    'fields' => array(
        array(
            'id'       => 'demo_user_select',
            'type'     => 'user_select',
...
KAVRO::createSection( $comment_prefix, array(
    'title'  => 'User Select Comment Field',
    'fields' => array(
        array(
            'id'       => 'demo_user_select',
            'type'     => 'user_select',
...
```php
$value = get_theme_mod( 'demo_user_select', null );
```

### From Taxonomy Options

```php
$value = kavro_get_term_meta( $term_id, $taxonomy_prefix, 'demo_user_select', null );
```

### From Profile/User Options

```php
$value = kavro_get_user_meta( $user_id, $profile_prefix, 'demo_user_select', null );
```

### From Nav Menu Options

```php
$value = kavro_get_nav_menu_item_meta( $menu_item_id, $nav_menu_prefix, 'demo_user_select', null );
```

### From Widget Options

```php
$value = kavro_get_widget_option( $widget_id, $widget_prefix, 'demo_user_select', null );
```

### From Comment Options

```php
$value = kavro_get_comment_meta( $comment_id, $comment_prefix, 'demo_user_select', null );
```
...
```php
$value = isset( $atts['demo_user_select'] ) ? $atts['demo_user_select'] : null;
```
