...
KAVRO::createSection( $prefix, array(
    ...
    'fields' => array(
        array(
            ...
            'type' => 'copy',
            'title' => 'Copy',
            'subtitle' => 'Copy field example.',
            'desc' => 'This is a documented copy field example.',
...
```php
$value = kavro_get_option( $prefix, 'demo_copy', null );
```

### From Metabox

```php
$value = kavro_get_post_meta( get_the_ID(), $metabox_prefix, 'demo_copy', null );
```
...
```php
$value = get_theme_mod( 'demo_copy', null );
```

### From Taxonomy Options

```php
$value = kavro_get_term_meta( $term_id, $taxonomy_prefix, 'demo_copy', null );
```

### From Profile/User Options

```php
$value = kavro_get_user_meta( $user_id, $profile_prefix, 'demo_copy', null );
```

### From Nav Menu Options

```php
$value = kavro_get_nav_menu_item_meta( $menu_item_id, $nav_menu_prefix, 'demo_copy', null );
```

### From Widget Options

```php
$value = kavro_get_widget_option( $widget_id, $widget_prefix, 'demo_copy', null );
```

### From Comment Options

```php
$value = kavro_get_comment_meta( $comment_id, $comment_prefix, 'demo_copy', null );
```
...
```php
$value = isset( $atts['demo_copy'] ) ? $atts['demo_copy'] : null;
```
