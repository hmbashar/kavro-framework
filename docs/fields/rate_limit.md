...
**Field type:** `rate_limit`  
**Renderer class:** `RateLimit`  
...
KAVRO::createSection( $prefix, array(
    ...
    'fields' => array(
        array(
            ...
            'type' => 'rate_limit',
            'title' => 'Rate Limit',
            'subtitle' => 'Rate Limit field example.',
            'desc' => 'This is a documented rate_limit field example.',
...
KAVRO::createSection( $nav_menu_prefix, array(
    'title'  => 'Rate Limit Menu Item Field',
    'fields' => array(
        array(
            'id'       => 'demo_rate_limit',
            'type'     => 'rate_limit',
...
KAVRO::createSection( $comment_prefix, array(
    'title'  => 'Rate Limit Comment Field',
    'fields' => array(
        array(
            'id'       => 'demo_rate_limit',
            'type'     => 'rate_limit',
...
```php
$value = kavro_get_option( $prefix, 'demo_rate_limit', null );
```

### From Metabox

```php
$value = kavro_get_post_meta( get_the_ID(), $metabox_prefix, 'demo_rate_limit', null );
```
...
```php
$value = get_theme_mod( 'demo_rate_limit', null );
```

### From Taxonomy Options

```php
$value = kavro_get_term_meta( $term_id, $taxonomy_prefix, 'demo_rate_limit', null );
```

### From Profile/User Options

```php
$value = kavro_get_user_meta( $user_id, $profile_prefix, 'demo_rate_limit', null );
```

### From Nav Menu Options

```php
$value = kavro_get_nav_menu_item_meta( $menu_item_id, $nav_menu_prefix, 'demo_rate_limit', null );
```

### From Widget Options

```php
$value = kavro_get_widget_option( $widget_id, $widget_prefix, 'demo_rate_limit', null );
```

### From Comment Options

```php
$value = kavro_get_comment_meta( $comment_id, $comment_prefix, 'demo_rate_limit', null );
```
...
```php
$value = isset( $atts['demo_rate_limit'] ) ? $atts['demo_rate_limit'] : null;
```
