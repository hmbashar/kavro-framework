...
**Renderer class:** `TextList`  
**Aliases using the same renderer:** None

## Purpose

Repeatable list of text strings.
...
KAVRO::createSection( $prefix, array(
    ...
    'fields' => array(
        array(
            ...
            'type' => 'text_list',
            'title' => 'Text List',
            'subtitle' => 'Text List field example.',
            'desc' => 'This is a documented text_list field example.',
...
KAVRO::createSection( $nav_menu_prefix, array(
    'title'  => 'Text List Menu Item Field',
    'fields' => array(
        array(
            'id'       => 'demo_text_list',
            'type'     => 'text_list',
...
```php
$value = kavro_get_option( $prefix, 'demo_text_list', null );
```

### From Metabox

```php
$value = kavro_get_post_meta( get_the_ID(), $metabox_prefix, 'demo_text_list', null );
```
...
```php
$value = get_theme_mod( 'demo_text_list', null );
```

### From Taxonomy Options

```php
$value = kavro_get_term_meta( $term_id, $taxonomy_prefix, 'demo_text_list', null );
```

### From Profile/User Options

```php
$value = kavro_get_user_meta( $user_id, $profile_prefix, 'demo_text_list', null );
```

### From Nav Menu Options

```php
$value = kavro_get_nav_menu_item_meta( $menu_item_id, $nav_menu_prefix, 'demo_text_list', null );
```

### From Widget Options

```php
$value = kavro_get_widget_option( $widget_id, $widget_prefix, 'demo_text_list', null );
```

### From Comment Options

```php
$value = kavro_get_comment_meta( $comment_id, $comment_prefix, 'demo_text_list', null );
```
...
```php
$value = isset( $atts['demo_text_list'] ) ? $atts['demo_text_list'] : null;
```
