...
KAVRO::createSection( $nav_menu_prefix, array(
    'title'  => 'Css Builder Menu Item Field',
    'fields' => array(
        array(
            'id'       => 'demo_css_builder',
            'type'     => 'css_builder',
...
KAVRO::createSection( $widget_prefix, array(
    'title'  => 'Css Builder Widget Field',
    'fields' => array(
        array(
            'id'       => 'demo_css_builder',
            'type'     => 'css_builder',
...
KAVRO::createSection( $comment_prefix, array(
    'title'  => 'Css Builder Comment Field',
    'fields' => array(
        array(
            'id'       => 'demo_css_builder',
            'type'     => 'css_builder',
...
```php
$value = get_theme_mod( 'demo_css_builder', null );
```

### From Taxonomy Options

```php
$value = kavro_get_term_meta( $term_id, $taxonomy_prefix, 'demo_css_builder', null );
```

### From Profile/User Options

```php
$value = kavro_get_user_meta( $user_id, $profile_prefix, 'demo_css_builder', null );
```

### From Nav Menu Options

```php
$value = kavro_get_nav_menu_item_meta( $menu_item_id, $nav_menu_prefix, 'demo_css_builder', null );
```

### From Widget Options

```php
$value = kavro_get_widget_option( $widget_id, $widget_prefix, 'demo_css_builder', null );
```

### From Comment Options

```php
$value = kavro_get_comment_meta( $comment_id, $comment_prefix, 'demo_css_builder', null );
```
...
```php
$value = isset( $atts['demo_css_builder'] ) ? $atts['demo_css_builder'] : null;
```
