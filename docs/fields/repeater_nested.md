...
**Field type:** `repeater_nested`  
**Renderer class:** `RepeaterNested`  
**Aliases using the same renderer:** None

## Purpose

Nested repeatable group builder.
...
KAVRO::createSection( $prefix, array(
    'title'  => 'Repeater Nested Example',
    'fields' => array(
        array(
            'id' => 'demo_repeater_nested',
            'type' => 'repeater_nested',
            'title' => 'Repeater Nested',
            'subtitle' => 'Repeater Nested field example.',
            'desc' => 'This is a documented repeater_nested field example.',
            'fields' => array(
                array(
                    'id' => 'item_title',
                    'type' => 'text',
...
KAVRO::createSection( $customize_prefix, array(
    'title'  => 'Repeater Nested Customizer',
    'fields' => array(
        array(
            'id'       => 'demo_repeater_nested',
            'type'     => 'repeater_nested',
...
KAVRO::createSection( $taxonomy_prefix, array(
    'title'  => 'Repeater Nested Term Field',
    'fields' => array(
        array(
            'id'       => 'demo_repeater_nested',
            'type'     => 'repeater_nested',
...
KAVRO::createSection( $profile_prefix, array(
    'title'  => 'Repeater Nested User Field',
    'fields' => array(
        array(
            'id'       => 'demo_repeater_nested',
            'type'     => 'repeater_nested',
...
KAVRO::createSection( $nav_menu_prefix, array(
    'title'  => 'Repeater Nested Menu Item Field',
    'fields' => array(
        array(
            'id'       => 'demo_repeater_nested',
            'type'     => 'repeater_nested',
...
KAVRO::createSection( $widget_prefix, array(
    'title'  => 'Repeater Nested Widget Field',
    'fields' => array(
        array(
            'id'       => 'demo_repeater_nested',
            'type'     => 'repeater_nested',
...
KAVRO::createSection( $comment_prefix, array(
    'title'  => 'Repeater Nested Comment Field',
    'fields' => array(
        array(
            'id'       => 'demo_repeater_nested',
            'type'     => 'repeater_nested',
...
```php
$value = kavro_get_option( $prefix, 'demo_repeater_nested', null );
```

### From Metabox

```php
$value = kavro_get_post_meta( get_the_ID(), $metabox_prefix, 'demo_repeater_nested', null );
```
...
```php
$value = get_theme_mod( 'demo_repeater_nested', null );
```

### From Taxonomy Options

```php
$value = kavro_get_term_meta( $term_id, $taxonomy_prefix, 'demo_repeater_nested', null );
```

### From Profile/User Options

```php
$value = kavro_get_user_meta( $user_id, $profile_prefix, 'demo_repeater_nested', null );
```

### From Nav Menu Options

```php
$value = kavro_get_nav_menu_item_meta( $menu_item_id, $nav_menu_prefix, 'demo_repeater_nested', null );
```

### From Widget Options

```php
$value = kavro_get_widget_option( $widget_id, $widget_prefix, 'demo_repeater_nested', null );
```

### From Comment Options

```php
$value = kavro_get_comment_meta( $comment_id, $comment_prefix, 'demo_repeater_nested', null );
```
...
```php
$value = isset( $atts['demo_repeater_nested'] ) ? $atts['demo_repeater_nested'] : null;
```
