...
**Renderer class:** `SystemInfo`  
**Aliases using the same renderer:** None

## Purpose

System information display field.
...
KAVRO::createSection( $prefix, array(
    ...
    'fields' => array(
        array(
            ...
            'type' => 'system_info',
            'title' => 'System Info',
            'subtitle' => 'System Info field example.',
            'desc' => 'This is a documented system_info field example.',
        ),
    ),
) );
```
...
KAVRO::createSection( $nav_menu_prefix, array(
    'title'  => 'System Info Menu Item Field',
    'fields' => array(
        array(
            'id'       => 'demo_system_info',
            'type'     => 'system_info',
...
KAVRO::createSection( $widget_prefix, array(
    'title'  => 'System Info Widget Field',
    'fields' => array(
        array(
            'id'       => 'demo_system_info',
            'type'     => 'system_info',
...
KAVRO::createSection( $comment_prefix, array(
    'title'  => 'System Info Comment Field',
    'fields' => array(
        array(
            'id'       => 'demo_system_info',
            'type'     => 'system_info',
...
```php
$value = kavro_get_option( $prefix, 'demo_system_info', null );
```

### From Metabox

```php
$value = kavro_get_post_meta( get_the_ID(), $metabox_prefix, 'demo_system_info', null );
```
...
```php
$value = get_theme_mod( 'demo_system_info', null );
```

### From Taxonomy Options

```php
$value = kavro_get_term_meta( $term_id, $taxonomy_prefix, 'demo_system_info', null );
```

### From Profile/User Options

```php
$value = kavro_get_user_meta( $user_id, $profile_prefix, 'demo_system_info', null );
```

### From Nav Menu Options

```php
$value = kavro_get_nav_menu_item_meta( $menu_item_id, $nav_menu_prefix, 'demo_system_info', null );
```

### From Widget Options

```php
$value = kavro_get_widget_option( $widget_id, $widget_prefix, 'demo_system_info', null );
```

### From Comment Options

```php
$value = kavro_get_comment_meta( $comment_id, $comment_prefix, 'demo_system_info', null );
```
...
```php
$value = isset( $atts['demo_system_info'] ) ? $atts['demo_system_info'] : null;
```
